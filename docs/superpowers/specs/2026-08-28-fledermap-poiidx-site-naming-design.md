...
| SN-3 | Fledermap ships its own curated `poiidx_filter_config.yaml` (toponymy-focused: place hierarchy + suburb/quarter/neighbourhood + forest/park + named water bodies/rivers), not poiidx's general-purpose default. **Widened 2026-09-01** against real field data: a real Hannover cluster showed most small stationary sites finding nothing but the whole city within the search radius, root-caused to real gaps in this file, not a rank-selection bug -- a genuine nearby park (`landuse=recreation_ground`) was never even indexed, since nothing in the filter asked poiidx to store it. Added the `place=` values poiidx's own `calculate_rank` already anticipates but this file never requested (`municipality`, `borough`, `farm`, `locality`, `isolated_dwelling`, `square` -- two new symbols, `locality` and `square`), plus `recreation_ground`/`garden`/`grass`/`meadow` (green space) and `canal`/`stream`/`drain`/`ditch`/`wetland`/`marsh` (water). Confirmed safe to broaden this far: poiidx's own scanner requires a plain `name` tag before filter_config is even consulted (`scanner.py`'s `poi_scan`, verified against source), so an unnamed generic feature (an anonymous meadow, say) can never match regardless of how broad the tag list is -- the noise concern that would otherwise argue for restraint here doesn't apply. Any change to this file forces poiidx to drop and recreate every table in `poiidx_bats_db` and re-scan from the Geofabrik `.pbf` (SN-1's hash-on-`init()` mechanism) -- deliberately batched into one change rather than adding tags as each gap is found. |
