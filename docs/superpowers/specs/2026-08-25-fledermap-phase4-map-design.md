...
| P4-9 | The parent spec's §9 default-hidden rule (`verdict IN ('noise', 'no_id')` excluded unless explicitly requested) is implemented by treating a recording with no non-superseded identification at all the same as an explicit `NO_ID` (§6) — referenced by this name from `map_query.py` and later specs since this phase, though never previously entered here. **Amended 2026-09-05** (`2026-09-05-fledermap-manual-classification-design.md`): once a genuine `NO_ID` can only come from a `MANUAL` claim, "no identification at all" and "a human confirmed nothing identifiable" are distinguishable for the first time and become separate filter options ("Unidentified" vs. "No ID") — the default (`SPECIES`-only) view this decision protects is unaffected. |
