...
def test_compute_peak_frequency_hz_ignores_a_louder_tone_below_the_search_window(
    ...
    the objectively loudest component in the file."""
    wav_path = tmp_path / "tone.wav"
    _sine_wav(wav_path, freq_hz=40_000.0)
    # Overlay a much louder 2kHz tone by writing a second file and mixing samples directly
    # would duplicate the sine-writer; simplest reproduction of "loud low-frequency noise
    # dominates a naive argmax" is a single strong low tone on its own -- if the window weren't
    # bounded, THIS tone would be reported as the peak.
    _sine_wav(wav_path, freq_hz=2_000.0, samplerate=256_000, duration_s=0.05)
